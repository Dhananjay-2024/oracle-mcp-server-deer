...
class SchemaManager(SchemaManagerProtocol):
    ...
    async def build_schema_index(self, fetch_all_metadata: bool = False) -> Dict[str, TableInfo]:
        """
        Build a schema index with table names and optionally full metadata.
        
        Args:
            fetch_all_metadata: If True, fetches complete metadata for all tables upfront.
                               If False (default), uses lazy loading for detailed information.
        """
        ...
        if fetch_all_metadata:
            print("Fetching complete metadata for all tables (this may take a while)...", file=sys.stderr)
            schema_index = {}
            
            # Fetch metadata for all tables
            for i, table_name in enumerate(all_table_names, 1):
                try:
                    if i % 10 == 0:
                        print(f"Progress: {i}/{len(all_table_names)} tables processed...", file=sys.stderr)
                    
                    table_details = await self.db_connector.load_table_details(table_name, fetch_all_metadata=True)
                    if table_details:
                        schema_index[table_name] = TableInfo(
                            ...
                            table_stats=table_details.get("table_stats"),
                            comments=table_details.get("comments"),
                            fully_loaded=True
                        )
                    else:
                        # Table exists in list but couldn't load details
                        schema_index[table_name] = TableInfo(
                            table_name=table_name,
                            columns=[],
                            relationships={},
                    ...
                    print(f"Error loading metadata for table {table_name}: {e}", file=sys.stderr)
            ...
            print(f"Complete metadata indexing finished for {len(schema_index)} tables", file=sys.stderr)
        else:
    ...
    async def load_or_build_cache(self, force_rebuild: bool = False, fetch_all_metadata: bool = False) -> SchemaCache:
        ...
        Args:
            force_rebuild: If True, rebuilds the cache from scratch
            fetch_all_metadata: If True during rebuild, fetches complete metadata for all tables
        """
        ...
        # Build new cache
        tables = await self.build_schema_index(fetch_all_metadata=fetch_all_metadata)
